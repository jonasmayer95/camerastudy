...
public class KinectInterop
{
    ...
    public struct JointData
    {
        ...
        internal void Copy(ref JointData jointData)
        {
            jointData.jointType = this.jointType;
            jointData.trackingState = this.trackingState;
            jointData.kinectPos = this.kinectPos;
            jointData.position = this.position;
            jointData.orientation = this.orientation;
            jointData.direction = this.direction;
            jointData.normalRotation = this.normalRotation;
            jointData.mirroredRotation = this.mirroredRotation;
        }
    }
    ...
    public struct BodyData
    {
        ...
        internal void Copy(ref BodyData bodyData)
        {
            bodyData.liTrackingID = this.liTrackingID;
            bodyData.position = this.position;
            bodyData.orientation = this.orientation;

            for (int i = 0; i < joint.Length; ++i)
            {
                joint[i].Copy(ref bodyData.joint[i]);
            }

            bodyData.normalRotation = this.normalRotation;
            bodyData.mirroredRotation = this.mirroredRotation;
            bodyData.hipsDirection = this.hipsDirection;
            bodyData.shouldersDirection = this.shouldersDirection;
            bodyData.bodyTurnAngle = this.bodyTurnAngle;
            bodyData.bodyFullAngle = this.bodyFullAngle;
            bodyData.isTurnedAround = this.isTurnedAround;
            bodyData.turnAroundFactor = this.turnAroundFactor;
            bodyData.leftHandOrientation = this.leftHandOrientation;
            bodyData.rightHandOrientation = this.rightHandOrientation;
            bodyData.headOrientation = this.headOrientation;
            bodyData.leftHandState = this.leftHandState;
            bodyData.leftHandConfidence = this.leftHandConfidence;
            bodyData.rightHandState = this.rightHandState;
            bodyData.rightHandConfidence = this.rightHandConfidence;
            bodyData.dwClippedEdges = this.dwClippedEdges;
            bodyData.bIsTracked = this.bIsTracked;
            bodyData.bIsRestricted = this.bIsRestricted;
        }
    }
